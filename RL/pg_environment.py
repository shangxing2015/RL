...
from config import _DEBUG_NO_NOISE, N_CHANNELS
import numpy as np
...
class Environment:
  ...
  def step(self, action, rec):
    ...
    for node_id in xrange(self.n_nodes):
      print node_id, action[0]
      channel_id = action[node_id] - 1

      if _DEBUG_NO_NOISE: # deterministic channel, change alternatively

        if rec % 3 == 1:
          conflict_matrix[channel_id][node_id] = 0

        else:
          conflict_matrix[channel_id][node_id] = 1
    ...
    observation  = np.array(observation).reshape(1,N_CHANNELS)
